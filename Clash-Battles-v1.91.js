...
let disputaVerCapturaJ1 = null;
let disputaVerCapturaJ2 = null;
  ...
  if (disputas.length > 0) {
    ...
    disputas.forEach(b => {
      html += `<tr><td data-label="ID:">#${b.id}</td><td data-label="J1">${b.j1Nombre} (${b.j1Tag})</td><td data-label="J2">${b.j2Nombre} (${b.j2Tag})</td><td data-label="Acción">
        <button class='btn btn-blue btn-sm' onclick='abrirVerificadorDisputa(${b.id}, "${b.j1Nombre}", "${b.j2Nombre}", "${b.j1ComprobanteDisputa || ''}", "${b.j2ComprobanteDisputa || ''}", "${b.j1MotivoDisputa || ''}", "${b.j2MotivoDisputa || ''}")'>🔍 Verificar</button>
      </td></tr>`;
    });
    html += '</tbody></table></div>';
  } else {
...
function abrirVerificadorDisputa(batallaId, j1Nombre, j2Nombre, capturaJ1, capturaJ2, motivoJ1, motivoJ2) {
  disputaVerBatallaId = batallaId;
  disputaVerCapturaJ1 = capturaJ1;
  disputaVerCapturaJ2 = capturaJ2;
  document.getElementById('disputaVerJ1').textContent = j1Nombre;
  document.getElementById('disputaVerJ2').textContent = j2Nombre;
  document.getElementById('disputaVerMotivoJ1').textContent = motivoJ1 || 'No especificado';
  ...
  if (!capturaJ1 || capturaJ1.trim() === '') {
    ...
    btnVerJ1.disabled = false;
    btnVerJ1.textContent = '🖼️ Ver Captura J1';
  }
  ...
  if (!capturaJ2 || capturaJ2.trim() === '') {
    ...
    btnVerJ2.disabled = false;
    btnVerJ2.textContent = '🖼️ Ver Captura J2';
  }
...
function renderRecargasAdmin() {
  ...
  let html = `<div class='table-wrapper'><table><thead><tr><th>ID</th><th>Usuario</th><th>Monto (USD)</th><th>Monto (Bs)</th><th>Referencia</th><th>Comprobante</th><th>Acciones</th></tr></thead><tbody>`;
  cacheRecargas.forEach(r => {
    const montoUSD = parseFloat(r.monto || 0);
    const montoBS = montoUSD * tasa;
    html += `<tr><td data-label="ID:">#${r.id}</td><td data-label="Usuario">${r.nombre} (${r.tag})</td><td data-label="Monto (USD)">$${montoUSD.toFixed(2)}</td><td data-label="Monto (Bs)">Bs ${formatVES(montoBS)}</td><td data-label="Referencia">${r.referencia}</td><td data-label="Comprobante">
  ...
  all.forEach(b => {
    ...
    if (b.estado === 'Pendiente de pago' && !b.j2Id && b.j1Id != userId) {
      ...
      if (soyCreador) {
        ...
        yaSubio = b.j2ComprobanteDisputa && b.j2ComprobanteDisputa.length > 0;
      }
      ...
      if (yaSubio) {
        accion = `
          <div style='display:flex; flex-direction:column; gap:6px; width:100%;'>
            ...
            <span style='color:var(--green); text-align:center; font-size:0.75rem;'>✅ Pruebas enviadas</span>
          </div>
        `;
      } else {
        accion = `
...
function mostrarModalGestionBatalla(id, miNom, miT, opNom, opT, opId, opLink, opTel, miDeclaracion) {
    ...
    if (miDeclaracion && miDeclaracion !== '') {
        ...
        btnDeclarar.style.display = 'flex';
    }
}
...
async function enviarRecarga() {
  ...
  if (res.success) {
    ...
    const bsSpan = document.getElementById('montoRecargaBs');
